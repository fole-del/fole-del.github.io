...
BokehShape.prototype.renderPath = function (ctx, renderer) {
  this.updateBokeh(); // set opacity

  if (renderer.isCanvas) {
    ctx.globalAlpha = this.getBokehAlpha();
  }

  Zdog.Shape.prototype.renderPath.apply(this, arguments); // reset opacity
...
function makeMadeline(isGood, colors, options) {
  ...
  [-1, 1].forEach(function (xSide) {
    var isLeft = xSide == 1; // shoulder ball
...
var featherGold = '#FE5'; // -- illustration shapes --- //
...
(function () {
  ...
  for (var i = 0; i < featherPartCount; i++) {
    ...
    var x = 4 - curve * 2;
    var y0 = sector / 2; // var y2 = -sector/2;
...
function update() {
  var now = new Date();
  var delta = now - then; // auto rotate
